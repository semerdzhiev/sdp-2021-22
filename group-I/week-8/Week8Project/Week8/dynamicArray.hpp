...
public:
    DynamicArray(size_t startSize = 10) : data(nullptr), startSize(startSize) {
        size_ = 0;
        capacity = (startSize == 0) ? 10 : startSize;
        data = new T[capacity];
    }

    DynamicArray(const DynamicArray<T>& other): data(nullptr), startSize(10) {
        copy(other);
    }
    ...
    void insert(T newValue, size_t position) {
        ...
        for (int movedElementIndex = size_ - 1; movedElementIndex >= position; --movedElementIndex) {
